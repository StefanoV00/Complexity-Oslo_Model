...
Llist = Llist_bulkz# np.array([32, 64, 128, 256, 512]) # Ls
...
T = T_bulkz; t = [i+1 for i in range(T)] # Max time T, time steps
Tlow = T_bulkz
...
for i in tqdm(range(len(Llist)), "Length"):
    ...
    hlist, hmean, hstd = pile.drive(Tlist[i], addspot = "random charge")
...
for i, L in tqdm(enumerate(Llist)):
    pl.figure("h vs t for different L 1")
    pl.plot(t[:len(hlist_list[i])], hlist_list[i], 
            label = f"L = {L}", zorder = 2 + 1/(i+1))
pl.figure("h vs t for different L 1")
pl.xlabel("t"); pl.ylabel("h"); pl.grid(alpha = 0.4 ) 

pl.figure("h vs t for different L 1")
pl.legend(loc = "lower right", fancybox=True)# shadow=True)
pl.subplots_adjust(left = 0.1, right = 0.980, top = 0.950 ) 
pl.savefig("plot_2a_1")  

del pile, hlist, hmean, hstd, slist, zs
protect = True

# np.save("bulkz_hlist_list", hlist_list, allow_pickle = True)
# np.save("bulkz_hmean_list", hmean_list, allow_pickle = True)
# np.save("bulkz_hstd_list",  hstd_list,  allow_pickle = True)
# np.save("bulkz_slist_list", slist_list, allow_pickle = True)

protect = True

#%% TASK (bulkz) 3a: Avalanche Probability
...
pl.yscale("log")
pl.grid(alpha = 0.4)
pl.savefig("bulkz_plot_3a_P(s;L)")
pl.show()
...
pl.yscale("log")
pl.grid(alpha = 0.4)
pl.savefig("bulkz_plot_3a_P(s;L)_collapse")
pl.show()
...
#%% TASK (bulkz) 3b: Avalanche Momenta
print("\nTASK (bulkz) 3b: Avalanche Momenta \n")
#% Non including s = 0
kmax = 10
...
pl.yscale("log")
pl.grid(alpha = 0.4)
pl.savefig("bulkz_plot_3b_scale")
del p
...
pl.grid(alpha = 0.4)
pl.savefig("bulkz_plot_3b_scale3")
...
pl.yscale("log")
pl.grid(alpha = 0.4)
pl.savefig("bulkz_plot_3b_P(s;L)_collapse_check")
pl.show()
